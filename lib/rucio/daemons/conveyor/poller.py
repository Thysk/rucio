            ...
            for activity in activities:
                ...
                if multi_vo:
                    for transf in transfs:
                        vo = transf['account'].vo
                        if vo not in xfers_ids:
                            xfers_ids[vo] = {}
                        if not transf[vo]['external_host'] in xfers_ids:
                            xfers_ids[transf['account'].vo].append(TransferParams(transf['external_id'], transf['request_id']))
...
def poll_transfers(external_host, xfers, request_ids=None, timeout=None, logger=logging.log, vo=None):
    ...
    try:
        ...
        try:
            ...
            resps = transfer_core.bulk_query_transfers(external_host, xfers, TRANSFER_TOOL, timeout, vo)

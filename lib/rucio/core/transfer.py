...
def bulk_query_transfers(request_host, transfer_ids, transfertool='fts3', timeout=None, logger=logging.log, vo=None):
    ...
    if transfertool == 'fts3':
        ...
        if vo:
            for vo in transfer_ids:
                for transfer_id in transfer_ids[vo]:
                    fts_resps = __return_fts_states_for_transfer_id(transfer_id, fts_resps)
        else:
...
def __return_fts_states_for_transfer_id(transfer_id, fts_resps):
